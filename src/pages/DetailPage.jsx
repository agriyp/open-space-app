...
function DetailPage() {
  const { id } = useParams();
  const { talkDetail = null, authUser } = useSelector((state) => state); // @DONE: get talkDetail and authUser state from store
  const dispatch = useDispatch(); // @DONE: get dispatch function from store

  useEffect(() => {
    // @DONE: dispatch async action to get talk detail by id
    dispatch(asyncReceiveTalkDetail(id));
  }, [id, dispatch]);

  const onLikeTalk = () => {
    // @DONE: dispatch async action to toggle like talk detail
    dispatch(asyncToogleLikeTalkDetail());
  };

  const onReplyTalk = (text) => {
    // @DONE: dispatch async action to add reply talk
